...
def _validate_section20_dispositions(dispositions: object) -> tuple[list[str], bool]:
    errors: list[str] = []
    has_blocker = False
    if not isinstance(dispositions, list):
        return ["section20_dispositions.json must contain a JSON array"], True
...
def validate_bundle(bundle_dir: Path) -> dict[str, object]:
    ...
    for required in ("release_id", "generated_at", "decision", "prerequisites", "critical_checks"):
    ...
    section20_errors, section20_has_blocker = _validate_section20_dispositions(section20_payload)
