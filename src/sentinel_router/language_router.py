...
def _load_fasttext_model():
    ...
    if not path.exists():
        logger.debug("SENTINEL_LID_MODEL_PATH does not exist: %s", path)
        return None
    try:
        fasttext = importlib.import_module("fasttext")
    except ModuleNotFoundError as exc:
        logger.debug("fastText module unavailable for LID routing: %s", exc)
        return None
    try:
        return fasttext.load_model(str(path))
    except Exception as exc:
        logger.debug("failed to load fastText LID model from %s: %s", path, exc)
        return None
